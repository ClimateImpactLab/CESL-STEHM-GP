...
%   Load catalogue file
catFile = fullfile(directoryName,'filelist_oct_readable.txt');
fid = fopen(catFile);
if fid == -1
    error(['Could not find catalogue file ',catFile])
end
txt = textscan(fid,'%n%f%f%s%n%n%s','delimiter', ';');
fclose(fid);
catalogue.id = txt{1};
...
for i = 1:noStations
    ...
    thisFileName = fullfile(directoryName,'data_oct_readable',fileList{i});
    fid = fopen(thisFileName);
    % txt = textscan(fid,'%11.4f;%6n;%2n;%1n%1n%1n');
    txt = textscan(fid,'%f%n%n%n%n%n', 'delimiter', ';');
    fclose(fid);
